...
function renderHtml(recs, meta) {
  ...
  var active = enriched.filter(function (x) { return x.flag === 'active'; }).length;
  var inactive = assigned - active;   // assigned seats not actively used (reclaim+watch+neverused)
...
'<div class="sub">Project <b>' + PROJECT + '</b> &middot; Last Update: <b>' + updated + '</b> &middot; reclaim threshold: idle &gt; ' + IDLE_RECLAIM_DAYS + 'd</div>',
'<div class="kpis">',
'<div class="kpi"><span class="ic">' + I_USERS + '</span><div><div class="n">' + assigned + ' / ' + LICENSE_CAP + '</div><div class="l">License Quota</div></div></div>',
'<div class="kpi green"><span class="ic">' + I_CLOCK + '</span><div><div class="n">' + active + '</div><div class="l">Active (&le;' + IDLE_WATCH_DAYS + 'd idle)</div></div></div>',
...
'<script>',
'var DATA=' + dataJson + ';',
'var sortK="idle",dir=-1;var tb=document.querySelector("#t tbody");',
'var collapsed={withlic:false,nolic:true};',  // No License collapsed by default
'function grp(r){return r.flag==="unlicensed"?"nolic":"withlic";}',
'function sortRows(a){return a.slice().sort(function(p,q){var x=p[sortK],y=q[sortK];',
'if(sortK==="idle"){x=x===""?1e9:+x;y=y===""?1e9:+y;}',  // never-logged-in = max idle (most inactive)
'return x<y?-1*dir:x>y?dir:0;});}',
'function esc(s){return String(s).replace(/[&<>]/g,function(c){return{"&":"&amp;","<":"&lt;",">":"&gt;"}[c];});}',
'function render(){',
'var g={withlic:[],nolic:[]};DATA.forEach(function(r){g[grp(r)].push(r);});',
'var out="";',
'[["withlic","With License"],["nolic","No License"]].forEach(function(G){',
'var k=G[0],lbl=G[1],rows=g[k],col=collapsed[k];',
'out+="<tr class=\\"grp\\" data-g=\\""+k+"\\"><td colspan=\\"4\\"><span class=\\"caret\\">"+(col?"\\u25B6":"\\u25BC")+"</span>"+lbl+"<span class=\\"cnt\\">("+rows.length+")</span></td></tr>";',
'if(!col){sortRows(rows).forEach(function(r){',
'out+="<tr class=\\""+r.flag+"\\"><td class=\\"email\\">"+esc(r.email)+"</td><td>"+(r.flag==="unlicensed"?"\\u2014":(r.assigned||"\\u2014"))+"</td><td>"+(r.last||"\\u2014")+"</td><td>"+(r.flag==="unlicensed"?"\\u2014":(r.idle===""?"<span class=\\"pill neverused\\">never</span>":"<span class=\\"pill "+r.flag+"\\">"+r.idle+"d</span>"))+"</td></tr>";});}});',
'tb.innerHTML=out;',
'document.querySelectorAll("tr.grp").forEach(function(tr){tr.onclick=function(){var k=tr.getAttribute("data-g");collapsed[k]=!collapsed[k];render();};});',
'}',
